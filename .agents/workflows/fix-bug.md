...
1. **Create Bug Branch** – The Orchestrator invokes the `kmp-branch` agent with the ticket identifier.
   ...
   ```
2. **Analysis & Debugging** – Analytical agents (e.g., `kmp-architect` or `Orchestrator`) investigate the bug reports, understand the root cause, and propose a code-level fix.
...
6. **Push Branch** – The `kmp-branch` agent pushes the final commit to `origin` and sets upstream.
7. **Open Pull Request** – The `kmp-pr` agent is invoked to create a PR via GitHub CLI.
   ```bash
